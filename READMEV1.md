...
* Gerenciamento de questões (Criação, edição e remoção);
* Cadastro de alternativas e definição da alternativa correta;
* Fluxo de correção:
...
* Visualização dos resultados;
* Estatísticas da turma e estatísticas individuais do aluno;
...
> **Nota:** A persistência e consulta dos dados serão realizadas através de integração direta da aplicação com o banco de dados, conforme definido para o projeto.
...
* **RF03 — Gerenciamento de questões:** O sistema deve permitir adicionar, editar e remover questões; cadastrar o enunciado; cadastrar alternativas; e definir uma alternativa como correta.
* **RF04 — Validação das questões:** O sistema deve realizar validações básicas durante o cadastro e edição das questões.
* **RF05 — Correção de provas:** O sistema deve disponibilizar um fluxo para iniciar a correção de uma prova. O fluxo principal deve contemplar: *Selecionar prova → QR Code → Gabarito → Captura das provas → Conclusão → Resultados*.
...
* **Correção:** Fluxo responsável por conduzir o usuário durante o processo de correção das provas. (Seleção da prova → QR Code → Gabarito → Captura → Conclusão).
...
Para verificar se o ambiente está configurado corretamente, rode:
```bash
flutter doctor
```
...
**Repositório:** [https://github.com/Klaus-E-J/flutter-mobile-app.git](https://github.com/Klaus-E-J/flutter-mobile-app.git)

Para clonar o projeto:
```bash
git clone https://github.com/Klaus-E-J/flutter-mobile-app.git
```
...
De forma geral, o projeto possui a seguinte estrutura:
...
[https://github.com/Klaus-E-J/flutter-mobile-app.git](https://github.com/Klaus-E-J/flutter-mobile-app.git)
